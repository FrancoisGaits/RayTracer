...
Scene *initScene42(int i, int nb_step) {
  Scene *scene = initScene();
  double coeff = (nb_step-i)/(double)nb_step;
  // setCamera(scene, point3( 4.5 + (10.f*coeff) , .8 + (8.f*coeff), 4.5 + (2.f*coeff)), vec3(0, 0.3, 0), vec3(0, 1, 0), 60,
  //           (float)WIDTH / (float)HEIGHT);

  setCamera(scene,point3(rotateZ(vec3(0, 1, 0) - vec3(4.5,.8,4.5), glm::pi<float>()*float(coeff)*0.5f)),vec3(0, 0.3, 0), vec3(0, 1, 0), 60,(float)WIDTH / (float)HEIGHT);
  setSkyColor(scene, color3(0.2, 0.2, 0.7));
  Material mat;
  mat.diffuseColor = color3(0.301, 0.034, 0.039);
  mat.specularColor = color3(1.0, 0.992, 0.98);
